...
CREATE TABLE IF NOT EXISTS problems (
  ...
  created_at timestamptz DEFAULT now(),
  -- 死活题特有字段
  problem_type text DEFAULT 'life',
  to_play text DEFAULT 'black'
);
...
-- =============================================
-- 【关键修复】创建 upsertUserProgress 函数
-- =============================================
CREATE OR REPLACE FUNCTION public.upsertUserProgress(
  p_user_id uuid,
...
AS $$
BEGIN
  -- 使用 ON CONFLICT 来实现 upsert 逻辑
  IF p_course_id IS NOT NULL THEN
    INSERT INTO public.learning_progress (user_id, course_id, progress, completed, last_accessed_at)
    VALUES (p_user_id, p_course_id, p_progress, p_completed, now())
    ON CONFLICT (user_id, course_id) DO UPDATE SET
      progress = EXCLUDED.progress,
      completed = EXCLUDED.completed,
      last_accessed_at = EXCLUDED.last_accessed_at;
  ELSIF p_problem_id IS NOT NULL THEN
    INSERT INTO public.learning_progress (user_id, problem_id, progress, completed, last_accessed_at)
    VALUES (p_user_id, p_problem_id, p_progress, p_completed, now())
