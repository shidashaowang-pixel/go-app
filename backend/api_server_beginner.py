...
围棋AI 初级API服务
================
使用训练好的PyTorch模型，配合极少MCTS搜索
比中级AI更弱，适合初级难度

启动方式:
  python api_server_beginner.py
"""
...
# ======================== 配置 ========================
BOARD_SIZE = 19
MODEL_PATH = os.environ.get('GO_MODEL_PATH', r'C:\Users\24816\Desktop\围棋AI训练\go_checkpoint.pth')
MCTS_ITERATIONS = int(os.environ.get('GO_MCTS_ITERS', '30'))  # 极少量搜索，更快更弱
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
PORT = int(os.environ.get('GO_API_PORT', '5002'))
...
if net is not None:
    net.eval()
    print(f"Model loaded | Device: {DEVICE} | MCTS iters: {MCTS_ITERATIONS} (beginner level)")
else:
...
def ai_move():
    ...
    try:
        ...
        # MCTS搜索：少量迭代 + 高随机性 = 更弱的AI
        move, probs = mcts_search(
            board, current, history, net,
            iterations=MCTS_ITERATIONS,
            temperature=1.2,  # 高温度 = 更多随机选择
            add_dirichlet=True,  # 加噪声
            size=size
        )
        ...
        if move is None:
            return jsonify({'x': -1, 'y': -1, 'pass': True, 'winrate': round(winrate, 4)})

        x, y = move
        print(f"[Beginner] AI move: ({x},{y}) | {dt:.2f}s")

        return jsonify({'x': x, 'y': y, 'pass': False, 'winrate': round(winrate, 4)})
...
def health():
    return jsonify({'status': 'ok', 'level': 'beginner', 'mcts_iters': MCTS_ITERATIONS})


if __name__ == '__main__':
    print(f"\n{'='*50}")
    print(f"  Go AI Beginner API Server")
    print(f"  MCTS: {MCTS_ITERATIONS} iterations (weak)")
    print(f"  Port: {PORT}")
    print(f"{'='*50}\n")
